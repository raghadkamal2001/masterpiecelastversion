...
export default function Collection() {
  const [books, setBooks] = useState([]);
  const [categories, setCategories] = useState(['كل الأقسام']);
  const [selectedCategory, setSelectedCategory] = useState('كل الأقسام');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [searchQuery, setSearchQuery] = useState('');
  ...
  const handleSearch = async () => {
    try {
      const response = await fetch(`/api/books/search/books?query=${searchQuery}`);
            ...
            <div className="mt-6 space-y-4">
              {searchResults.map((book) => (
                <div key={book._id} className="p-4 bg-white rounded shadow">
                  ...
                  <p className="text-sm">{book.summary?.slice(0, 100)}...</p>
                </div>
              ))}
            </div>
          )}
    ...
    <div className="container mx-auto px-4 py-8" dir="rtl">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {books.map((book, index) => (
          ...
          >
            <div className="mb-3 text-gray-800 transition-transform duration-300 group-hover:scale-110">
              <BookOpen className="w-6 h-6 transition-colors duration-300 hover:text-orange-400" />
            </div>
            <h3 className="text-lg font-bold mb-2 text-gray-800 transition-colors duration-300 hover:text-orange-500">
              {book.title}
            </h3>
            ...
            <button
              onClick={() => handleClick(book._id)}
              className="bg-orange-400 hover:bg-orange-600 text-white py-2 px-6 rounded-full text-sm transition-all duration-300 hover:px-8"
            >
              إقرأ المزيد
            </button>
          </div>
